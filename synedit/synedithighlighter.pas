...
$Id: synedithighlighter.pp 34411 2011-12-25 19:00:46Z juha $
...
uses
  System.SysUtils, System.Classes, Winapi.Windows, LazMethodList,
  System.IOUtils, Registry, IniFiles, System.UITypes,
...
type
  ...
  public
    ...
    property BoldPriority: integer index TFontStyle.fsBold read GetStylePriority write SetStylePriority default 0;
    property ItalicPriority: integer index TFontStyle.fsItalic read GetStylePriority write SetStylePriority default 0;
    property UnderlinePriority: integer index TFontStyle.fsUnderline read GetStylePriority write SetStylePriority default 0;
  end;
...
type
  ...
  public
    ...
    procedure SetLine(NewValue: String;
...
begin
  BeginUpdate;
  Background := TColorRec.Null;
  Foreground := TColorRec.Null;
  FrameColor := TColorRec.Null;
  FrameStyle := slsSolid;
...
begin
  ...
  FrameStyle := slsSolid;
  FrameEdges := sfeAround;
  FBackgroundDefault := TColorRec.Null;
  FForegroundDefault := TColorRec.Null;
  FFrameColorDefault := TColorRec.Null;
...
{$IFNDEF SYN_LAZARUS}
const
  Pal16: array [0..15] of TColor = (TColorRec.Black, TColorRec.Maroon, TColorRec.Green, TColorRec.Olive,
          TColorRec.Navy, TColorRec.Purple, TColorRec.Teal, TColorRec.LtGray, TColorRec.DkGray, TColorRec.Red, TColorRec.Lime,
          TColorRec.Yellow, TColorRec.Blue, TColorRec.Fuchsia, TColorRec.Aqua, TColorRec.White);
{$ENDIF}
          ...
          if OpenKeyReadOnly(attrKey) then begin
            try
              if ValueExists(attrName) then begin
                ...
                if bgDefault = '1'
                  then Background := TColorRec.cWINDOW
                  else Background := Pal16[StrToInt(bgIndex16)];
                if fgDefault = '1'
                  then Foreground := TColorRec.cWindowText
                  else Foreground := Pal16[StrToInt(fgIndex16)];
                Style := [];
                if Pos('B',fontStyle) > 0 then Style := Style + [TFontStyle.fsBold];
                if Pos('I',fontStyle) > 0 then Style := Style + [TFontStyle.fsItalic];
                if Pos('U',fontStyle) > 0 then Style := Style + [TFontStyle.fsUnderline];
                Result := true;
              end;
        ...
        with reg do begin
          ...
          if OpenKeyReadOnly(attrKey+'\'+attrName) then begin
            try
              ...
              Style := [];
              if IsTrue(fontBold) then Style := Style + [TFontStyle.fsBold];
              if IsTrue(fontItalic) then Style := Style + [TFontStyle.fsItalic];
              if IsTrue(fontUnderline) then Style := Style + [TFontStyle.fsUnderline];
              Result := true;
            finally CloseKey; end;
          end; // if
          {$ENDIF}
  ...
  try
    ...
    begin
      if S.IndexOf('Background') <> -1 then
        Background := Ini.ReadInteger(StoredName, 'Background', TColorRec.cWindow);
      if S.IndexOf('Foreground') <> -1 then
        Foreground := Ini.ReadInteger(StoredName, 'Foreground', TColorRec.cWindowText);
...
begin
  if TFontStyle.fsBold in Style then Result:= 1 else Result:= 0;
  if TFontStyle.fsItalic in Style then Result:= Result + 2;
  if TFontStyle.fsUnderline in Style then Result:= Result + 4;
  if TFontStyle.fsStrikeout in Style then Result:= Result + 8;
end;

procedure TSynHighlighterAttributes.SetStyleFromInt(const Value: integer);
begin
  if Value and $1 = 0 then  Style:= [] else Style:= [TFontStyle.fsBold];
  if Value and $2 <> 0 then Style:= Style + [TFontStyle.fsItalic];
  if Value and $4 <> 0 then Style:= Style + [TFontStyle.fsUnderline];
  if Value and $8 <> 0 then Style:= Style + [TFontStyle.fsStrikeout];
end;

function TSynHighlighterAttributes.GetStyleMaskFromInt : integer;
begin
  if TFontStyle.fsBold in StyleMask then Result:= 1 else Result:= 0;
  if TFontStyle.fsItalic in StyleMask then Result:= Result + 2;
  if TFontStyle.fsUnderline in StyleMask then Result:= Result + 4;
  if TFontStyle.fsStrikeout in StyleMask then Result:= Result + 8;
end;

procedure TSynHighlighterAttributes.SetStyleMaskFromInt(const Value : integer);
begin
  if Value and $1 = 0 then  StyleMask:= [] else StyleMask:= [TFontStyle.fsBold];
  if Value and $2 <> 0 then StyleMask:= StyleMask + [TFontStyle.fsItalic];
  if Value and $4 <> 0 then StyleMask:= StyleMask + [TFontStyle.fsUnderline];
  if Value and $8 <> 0 then StyleMask:= StyleMask + [TFontStyle.fsStrikeout];
end;
...
procedure TSynCustomHighlighter.SetLine(NewValue: String; LineNumber: Integer);
begin
...
begin
  inherited;
  FDepth := 0;
  FTopSetting.Color := TColorRec.SystemColor;
  FNestSetting.Color := TColorRec.SystemColor;
end;
