...
uses
  System.Classes, System.SysUtils, FMX.Controls, FMX.Menus, math, SynEditMarks, FMX.Forms, FMX.Types, System.Types,
  SynEditMiscClasses, SynTextDrawer, SynEditMouseCmds, SynEditFoldedView, System.UITypes, LazMethodList;

type

  TGutterClickEvent = procedure(Sender: TObject; X, Y:Single; Line: integer;
  ...
  public
    ...
    property Color: TColor read FColor write SetColor default TColorRec.cBtnFace;
  ...
  public
    ...
    constructor Create(AOwner: TComponent; AGutter: TSynGutterBase); overload;
    destructor  Destroy; override;
  ...
  public
    ...
    procedure ResetMouseActions; virtual; // set mouse-actions according to current Options / may clear them
    procedure DoOnGutterClick(X, Y: Single);  virtual;
...
begin
  ...
  case FSide of
    gsLeft:
      begin
        NewLeft   := 0;
        NewHeight := Trunc(SynEdit.Height);
      end;
    gsRight:
      begin
        NewLeft   := Trunc(SynEdit.Width - Width - ScrollBarWidth);
        NewHeight := Trunc(SynEdit.Height);
      end;
  end;
...
procedure TSynGutterBase.DoDefaultGutterClick(Sender: TObject; X, Y:Single; Line: integer; mark: TSynEditMark);
begin
end;
...
begin
  ...
  FMarkupInfo.Background := TColorRec.cBtnFace;
  FMarkupInfo.Foreground := 0;
  FMarkupInfo.FrameColor := 0;
...
procedure TSynGutterPartBase.MouseMove(Shift: TShiftState; X, Y: Single);
begin
end;

procedure TSynGutterPartBase.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
end;
...
procedure TSynGutterPartBase.DoOnGutterClick(X, Y : Single);
begin
