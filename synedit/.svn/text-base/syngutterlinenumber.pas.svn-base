...
uses
  System.Classes, System.SysUtils, SynGutterBase, FMX.Types, System.Types,
...
//  FTextDrawer.BeginDrawing(dc);
  try
    if MarkupInfo.Background <> TColorRec.Null then
      FTextDrawer.SetBackColor(MarkupInfo.Background)
    else
      FTextDrawer.SetBackColor(Gutter.Color);
    if MarkupInfo.Foreground <> TColorRec.Null then
...
//      fTextDrawer.SetForeColor(TCustomSynEdit(SynEdit).Font.Color);
    ...
    begin
      ...
      fTextDrawer.ExtTextOut(rcLine.Left, rcLine.Top, {ETO_OPAQUE or ETO_CLIPPED}0, rcLine,
    ...
    begin
      ...
      with rcLine do
        fTextDrawer.ExtTextOut(Left, Top, {ETO_OPAQUE}0, rcLine, nil, 0);
    end;
  finally
