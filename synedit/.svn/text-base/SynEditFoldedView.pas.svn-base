...
uses
  System.Classes, System.SysUtils, LazSynEditText, SynEditTypes, SynEditMiscClasses,
...
type
  ...
  public
    ...
    function Precessor : TSynTextFoldAVLNodeData; reintroduce; overload;
    function Successor : TSynTextFoldAVLNodeData; reintroduce; overload;
    function Precessor(var aStartPosition, aSizesBeforeSum : Integer) : TSynTextFoldAVLNodeData; reintroduce; overload;
    function Successor(var aStartPosition, aSizesBeforeSum : Integer) : TSynTextFoldAVLNodeData; reintroduce; overload;
  end;

  { TSynTextFoldAVLNode }

  TSynTextFoldAVLNode = record
...
//  protected
  ...
  public
    ...
    function Equals(AnInfo: TSynFoldNodeInfo): Boolean; overload;
    function Equals(AHelper: TSynFoldNodeInfoHelper): Boolean; overload;
...
type
  ...
  public
    constructor Create(AFoldType: Pointer); overload;
    constructor Create(AStream: TSynEditFoldExportStream); overload;
    destructor Destroy; override;
...
begin
  ...
  for i := 0 to FLen - 1 do
    c := c xor (ord((FMem + i)^) * (i+1));
  c := (c mod 256) xor ((c div 256) mod 256) xor ((c div 65536) mod 256);
  Result := Byte((Pointer(LongInt(FMem) + FLen))^) = Byte(NumEncode86Chars[1 + (c mod 86)]);
end;
...
begin
  ...
  FExportStream.AppendNum(Integer(AFoldType));
  FFoldType := AFoldType;
  FWriteCacheLen := 0;
  FWriteCache := nil;
  FWriteCacheTypes := [];
...
begin
  ...
  FFoldType := Pointer(Integer(AStream.ReadNum));
  while(true) do begin
    i := AStream.FindChar(' ');
...
var
  ...
  procedure NestCurrentIntoNewBlock;
  var
...
begin
  ...
  FMarkupInfoFoldedCode.Background := TColorRec.Null;
  FMarkupInfoFoldedCode.Foreground := TColorRec.DkGray;
  FMarkupInfoFoldedCode.FrameColor := TColorRec.DkGray;
