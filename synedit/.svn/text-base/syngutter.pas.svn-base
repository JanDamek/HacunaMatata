...
uses
  System.SysUtils, System.Classes, FMX.Controls, FMX.Menus, System.Types, System.UITypes, FMX.Types,
...
type
  ...
  protected
    procedure DoDefaultGutterClick(Sender: TObject; X, Y:Single; Line: integer;
  ...
  public
    ...
    procedure DoOnGutterClick(X, Y: Single);
...
begin
  ...
  ScreenRow1 := Max(Trunc((AClip.Top - Bounds.Top) / TextArea.LineHeight), 0);
  ScreenRow2 := Min(Trunc((AClip.Bottom-1 - Bounds.Top) / TextArea.LineHeight), TextArea.LinesInWindow + 1);
...
procedure TSynGutter.DoDefaultGutterClick(Sender: TObject; X, Y:Single; Line: integer;
  mark: TSynEditMark);
begin
end;
...
procedure TSynGutter.DoOnGutterClick(X, Y: Single);
begin
  Parts[PixelToPartIndex(Trunc(X))].DoOnGutterClick(X, Y);
end;
...
//  TextDrawer.SetForeColor(TCustomSynEdit(SynEdit).Font.Color);
  TextDrawer.SetFrameColor(TColorRec.Null);
   with AClip do
     TextDrawer.ExtTextOut(Left, Top, 0, AClip, nil, 0);
...
procedure TSynGutter.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  FMouseDownPart := PixelToPartIndex(Trunc(X));
  Parts[FMouseDownPart].MouseDown(Button, Shift, trunc(X), trunc(Y));
  if (Button=TMouseButton.mbLeft) then
    DoOnGutterClick(X, Y);
end;

procedure TSynGutter.MouseMove(Shift: TShiftState; X, Y: Single);
begin
  Parts[FMouseDownPart].MouseMove(Shift, X, Y);
end;

procedure TSynGutter.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
...
begin
  MouseDownPart := PixelToPartIndex(Trunc(AnInfo.MouseX));
...
begin
  ...
  MarkupInfo.Background := TColorRec.White;
  MarkupInfo.Foreground := TColorRec.DkGray;
  FLineWidth := 1;
  FLineOffset := 0;
  FLineOnRight := True;
end;
