...
uses
  LazSynEditMouseCmdsTypes, System.Classes, FMX.Controls, System.SysUtils, SynEditStrConst, SynEditPointClasses,
  FMX.Dialogs, System.UITypes, LazMethodList;
...
const
  ...
  SynMouseButtonMap: Array [TMouseButton] of TSynMouseButton =
    (mbXLeft, mbXRight, mbXMiddle);

  SynMouseButtonBackMap: Array [TSynMouseButton] of TMouseButton =
    (TMouseButton.mbLeft, TMouseButton.mbRight, TMouseButton.mbMiddle,
     TMouseButton.mbLeft, TMouseButton.mbLeft, TMouseButton.mbLeft, TMouseButton.mbLeft);
...
type
  ...
  TSynEditMouseActionInfo = record
    ...
    Shift: TShiftState;
    MouseX, MouseY, WheelDelta: Single;
  ...
  while i > 0 do begin
    try
      ...
      on E : ESynMouseCmdError do begin
        Delete(i);
        if assigned(fOwner) and (csDesigning in TComponent(Owner).ComponentState)
        then
          MessageDlg(SYNS_EDuplicateShortCut + LineEnding + E.Message + LineEnding + Items[i].DisplayName,
