...
uses
  SysUtils, Classes, math, SynEditHighlighter, SynEditTypes, AvgLvlTree;
...
type
  ...
  public
    ...
    function FoldBlockOpeningCount(ALineIndex: TLineIdx;
                                   const AFilter: TSynFoldBlockFilter): integer; overload; virtual;
...
var
  i: LongInt;
begin
  if HighlighterRanges=nil then HighlighterRanges:=TList.Create;
...
begin
  ...
  for i := low(TSynFoldAction) to high(TSynFoldAction) do
    if i in AFoldActions then begin
      s:=  GetEnumName(TypeInfo(TSynFoldAction), integer(i)) ;
      Result := Result + s + ',';
    end;
    ...
    if sfaInvalid in FoldAction then
      ...
      Result := Format('L=%3d I=%d  X=%2d-%2d  Fld=%d-%d Nst=%d-%d  FT=%d FTC=%d  Grp=%d  A=%s',
                       [LineIndex, NodeIndex, LogXStart, LogXEnd,
                        ...
                        Integer(FoldType), Integer(FoldTypeCompatible), FoldGroup,
                        dbgs(FoldAction)]);
end;
...
procedure TSynCustomFoldHighlighter.SetLine(NewValue: String;
  LineNumber: Integer);
begin
  inherited;
...
function TSynCustomCodeFoldBlock.GetOrCreateSibling(ABlockType: Pointer): TSynCustomCodeFoldBlock;
  ...
  begin
    ...
    while N.FBlockType <> t do begin
      ...
      inc(i);
      if LongInt(t) < LongInt(N.FBlockType)
      then N := N.FLeft
      else N := N.FRight;
    end;
    if i >= l then begin
...
begin
  ...
  while (assigned(Result)) do begin
    if Result.FBlockType = ABlockType then
      exit;
    P := Result;
    if LongInt(ABlockType) < LongInt(Result.FBlockType)
  ...
  if LongInt(ABlockType) < LongInt(P.FBlockType) then begin
    P.FLeft := Result;
    dec(P.FBalance);
  end else begin
    P.FRight := Result;
...
begin
  if (Template<>nil) and (ClassType<>Template.ClassType) then
    raise EClassNotFound.Create('Nespravna trida');
...
begin
  if LongInt(RangeType) < LongInt(Range.RangeType) then
    Result:=1
  else if LongInt(RangeType) > LongInt(Range.RangeType) then
    Result:=-1
  else if LongInt(Pointer(FTop)) < LongInt(Pointer(Range.FTop)) then
    Result:= -1
  else if LongInt(Pointer(FTop)) > LongInt(Pointer(Range.FTop)) then
    Result:= 1
  else
...
function TSynCustomHighlighterRange.Add(ABlockType: Pointer = nil; IncreaseLevel: Boolean = True):
        TSynCustomCodeFoldBlock;
var
  i: LongInt;
begin
...
begin
//  debugln('TSynCustomHighlighterRange.WriteDebugReport ',DbgSName(Self), ' RangeType=',dbgs(RangeType),' StackSize=',dbgs(CodeFoldStackSize));
