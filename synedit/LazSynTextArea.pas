...
uses
  System.Classes, System.SysUtils, FMX.Controls, System.UITypes, System.UIConsts,
  SynEditTypes, SynEditMiscProcs, SynEditMiscClasses, LazSynEditText, System.Types,
  SynEditMarkup, SynEditHighlighter, SynTextDrawer, FMX.Forms, FMX.Types;
...
type
  ...
  public
    constructor Create(AOwner: TControl; ATextDrawer: TheTextDrawer);
...
constructor TLazSynSurfaceManager.Create(AOwner: TControl);
begin
...
constructor TLazSynTextArea.Create(AOwner: TControl; ATextDrawer: TheTextDrawer);
var
  i: TLazSynBorderSide;
begin
  inherited Create(AOwner);
  ...
  FRightEdgeColor   := TColorRec.Silver;
  FontChanged;
end;
    ...
    if fTextDrawer.UseUTF8 then begin
      while SrcPos<Count do begin
        ...
        if c = #9 then begin
          ...
          // could be UTF8 char
          if c in [#128..#255]
          then CharLen := {UTF8CharacterStrictLength(@p[SrcPos])}1
          else CharLen := 1;
  ...
  begin
    ...
    begin
      ...
      if rcToken.Right > AClip.Right then begin
        ...
        fTextDrawer.FrameColor[bsRight] := TColorRec.Null; // right side of char is not painted
      end;
    ...
    if bFillToEOL and (rcToken.Left < rcLine.Right) then begin
      ...
      repeat
        ...
        begin
          ...
          if FillFCol = TColorRec.Null then FillFCol := ForegroundColor;
          if FillBCol = TColorRec.Null then FillBCol := colEditorBG;
        end else
        begin
          FillFCol := ForegroundColor;
          FillBCol := colEditorBG;
          FillFrame := TColorRec.Null;
  ...
  begin
    ...
    begin
      CanAppend :=
        ...
        (TokenAccu.FrameColor[bsRight] = TColorRec.Null) and
        (MarkupInfo.FrameSideColors[bsLeft] = TColorRec.Null) and
        // colors
        ...
        ( ( (TokenAccu.FG = MarkupInfo.Foreground) and
          ...
          ( (TokenAccu.Style - [TFontStyle.fsBold, TFontStyle.fsItalic] = MarkupInfo.Style - [TFontStyle.fsBold, TFontStyle.fsItalic]) and
            ( (TokenAccu.Style * [TFontStyle.fsUnderline, TFontStyle.fsStrikeOut] = []) or
...
//        debugln('ERROR: Could not convert CharLen (',dbgs(aCharLen),') to byteLen (maybe invalid UTF8?)',' len ',dbgs(nTokenByteLen),' Line ',dbgs(CurLine),' PhysPos ',dbgs(CurPhysPos));
    ...
    begin
      ...
      begin
        ...
        if DefaultBGCol = TColorRec.Null then DefaultBGCol := colEditorBG;
        if DefaultFGCol = TColorRec.Null then DefaultFGCol := ForegroundColor;
        ...
        if FPaintLineColor.Background = TColorRec.Null then
          FPaintLineColor.Background := colEditorBG;
        if FPaintLineColor.Foreground = TColorRec.Null then
...
//      InternalFillRect(dc, rcLine);
      ...
      if (DividerInfo.Color <> TColorRec.Null) and (nRightEdge >= FTextBounds.Left) then
      begin
        ypos := rcToken.Bottom - 1;
        cl := DividerInfo.Color;
        if cl = TColorRec.cDESKTOP then
          cl := RightEdgeColor;
...
{  Canvas.Color := RightEdgeColor; // used for code folding too
  Canvas.Pen.Width := 1;}
