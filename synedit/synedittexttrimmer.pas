...
begin
  ...
  if  fLockCount > 0 then begin
    ...
    for i := fLockList.Count-1 downto 0 do begin
      j := Integer(Integer(Pointer(fLockList.Objects[i])));
...
begin
  ...
  if fLockCount > 0 then begin
    i := fLockList.IndexOfObject(TObject(pointer(Integer(Index))));
    if i < 0 then
      fLockList.AddObject(SpaceStr, TObject(pointer(Integer(Index))))
    else
...
begin
  if (fLockCount = 0) and (fLineIndex >= 0) and Enabled then begin
    fLockList.AddObject(Spaces(fLineIndex), TObject(Pointer(Integer(fLineIndex))));
    FLineEdited := False;
  end;
  inc(fLockCount);
end;
...
begin
  ...
  i := fLockList.IndexOfObject(TObject(Pointer(Integer(fLineIndex))));
  if i >= 0 then begin
  ...
  try
    for i := 0 to fLockList.Count-1 do begin
      index := Integer(Integer(Pointer(fLockList.Objects[i])));
...
begin
  ...
  if (fLockCount > 0) and (fLockList.IndexOfObject(TObject(Pointer(Integer(ALineIndex)))) < 0) then exit;
