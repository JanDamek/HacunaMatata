...
uses
  System.Classes, FMX.Controls, System.sysutils, math, System.UITypes, FMX.Types, System.Types,
...
type
  ...
  TSynGutterLOvMarkList = class(TList)
  private
    FOwnMarks: Boolean;
    FLockCount: Integer;
    FNeedSort: Boolean;
  ...
  TSynGutterLOvLineMarksList = class(TList)
  private
    FItemHeight: Integer;
  ...
  TSynChildWinControl = class(TControl)
  public
    constructor Create(AOwner: TComponent); override;
  end;
  ...
  protected
    ...
    Procedure PaintWinControl(Sender: TObject; Canvas: TCanvas; const ARect: TRectF);
  public
    destructor Destroy; override;
...
begin
  ...
  if Result <> 0 then exit;
  Result := Integer(self) - Integer(Other);
end;
...
begin
  ...
  FColor := AValue;
  FRGBColor := TColorRec.ColorToRGB(AValue);
  DoChange(Self);
end;
...
procedure TSynGutterLOvProviderCurrentPage.Paint(Canvas: TCanvas; AClip: TRect; TopOffset: integer);
var R:TRectF;
begin
...
//  Canvas.Brush.Color := FRGBColor;
  r.create(AClip);
  Canvas.FillRect(r,0,0,[],0,TCornerType.ctRound);
end;
...
begin
  ...
  FColorSaved := AValue;
  FRGBColorSaved := TColorRec.ColorToRGB(AValue);
  DoChange(Self);
end;
...
//      else
//        Canvas.Brush.Color := FRGBColor;
      Canvas.FillRect(TRectF.create(AClip),0,0,[],0,TCornerType.ctRound);
    end;
...
begin
  ...
  //TODO: predelat registraci eventu
...
procedure TSynGutterLineOverview.PaintWinControl(Sender: TObject; Canvas: TCanvas; const ARect: TRectF);
var
  i: Integer;
  AClip: TRect;
begin
  if not Visible then exit;
//  AClip := FWinControl.Canvas;
  AClip.Left := 0;
  AClip.Right := Width;
//  FWinControl.Canvas.Brush.Color := MarkupInfo.Background;
  FWinControl.Canvas.FillRect(TRectF.create(AClip),0,0,[],0,TCornerType(0));
