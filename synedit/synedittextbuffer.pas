...
$Id: synedittextbuffer.pp 34979 2012-01-27 17:00:59Z martin $
...
begin
  Result := 'X='+IntToStr(FPosX) + ' Y='+ IntToStr(FPosY) + ' len=' + IntToStr(FLen);
end;
...
begin
  Result := 'X='+IntToStr(FPosX) + ' Y='+ IntToStr(FPosY) + ' text=' + FText;
end;
...
begin
  Result := 'X='+IntToStr(FPosX) + ' Y='+ IntToStr(FPosY);
end;
...
begin
  Result := 'Y='+IntToStr(FPosY) + ' Cnt='+ IntToStr(FCount);
end;
...
begin
  ...
  FAttachedSynEditList := TList.Create;
...
begin
  P := ItemPointer[Index];
  P := Pointer(LongInt(P)+SizeOf(String) + SizeOf(TObject));
  (PSynEditStringFlags(P ))^ := AValue;
end;
...
begin
  P := ItemPointer[Index];
  P := Pointer(LongInt(P)+ SizeOf(String));
  Result := (PObject(P ))^;
end;
