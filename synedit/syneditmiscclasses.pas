...
$Id: syneditmiscclasses.pp 35366 2012-02-14 20:01:45Z martin $
...
uses
  System.UITypes, System.UIConsts, FMX.Types, System.Types,
  Classes, FMX.Controls, SysUtils, SynEditHighlighter, LazMethodList, FMX.Memo,
...
type
  ...
  public
    ...
    property OnChange: TNotifyEvent read fOnChange write fOnChange;
  published
    property Background default TColorREc.cHIGHLIGHT;
    property Foreground default TColorREc.cHighLightText;
    property FrameColor default TColorREc.Null;
...
//    function GetHandle: THandle;
    ...
    function getFBoundsBottom: integer;
    function getFBoundsLeft: Integer;
    function getFBoundsRight: Integer;
    function getFBoundsTop: Integer;
    function getRectFromBounds: TRect;
  protected
...
//    property  Handle: THandle read GetHandle;
  public
    ...
    property Left: Integer   read getFBoundsLeft;
    property Top: Integer    read getFBoundsTop;
    property Right:Integer   read getFBoundsRight;
    property Bottom: integer read getFBoundsBottom;
    property Bounds: TBounds read FBounds;
    property Rect: TRect read getRectFromBounds;
  ...
  private
    ...
    fXoffset: integer;
    fOnChange: TNotifyEvent;
    procedure SetBookmarkImages(const Value: TList);
...
(*  TSynClipboardStream = class
  ...
  protected
    ...
    function IndexOf(AHandler: TMethod): Integer; overload;
    function IndexOf(AHandler: TMethod; AFilter: LongInt): Integer; overload;
  ...
  public
    ...
    procedure Remove(AHandler: TMethod); overload;
    procedure Remove(AHandler: TMethod; AFilter: LongInt); overload;
...
type
  ...
  public
    ...
    function Precessor: TSynSizedDifferentialAVLNode; overload;
    function Successor: TSynSizedDifferentialAVLNode; overload;
    function Precessor(var aStartPosition, aSizesBeforeSum : Integer): TSynSizedDifferentialAVLNode; overload;
    function Successor(var aStartPosition, aSizesBeforeSum : Integer): TSynSizedDifferentialAVLNode; overload;
  end;
  ...
  protected
    ...
    procedure SetRoot(ANode : TSynSizedDifferentialAVLNode); overload; virtual;
    procedure SetRoot(ANode : TSynSizedDifferentialAVLNode; anAdjustChildPosOffset : Integer); overload; virtual;
  ...
  public
    ...
    function First: TSynSizedDifferentialAVLNode; overload;
    function Last: TSynSizedDifferentialAVLNode; overload;
    function First(out aStartPosition, aSizesBeforeSum : Integer): TSynSizedDifferentialAVLNode; overload;
    function Last(out aStartPosition, aSizesBeforeSum : Integer): TSynSizedDifferentialAVLNode; overload;
  end;
...
begin
  ...
  Background := TColorRec.cHighLight;
  Foreground := TColorRec.cHighLightText;
end;
...
begin
  if Foreground <> TColorRec.Null then AForeground := Foreground;
  if Background <> TColorRec.Null then ABackground := Background;
  if FrameColor <> TColorRec.Null then
  begin
...
begin
  ...
  else if FrameColor = TColorRec.Null
  then Result := sfeNone
...
begin
  ...
  if (Other.Background <> TColorRec.Null) and (Other.BackPriority >= BackPriority) then begin
    Background := Other.Background;
    BackPriority := Other.BackPriority;
  end;
  if (Other.Foreground <> TColorRec.Null) and (Other.ForePriority >= ForePriority) then begin
...
procedure TSynSelectedColor.MergeFrames(Other: TSynSelectedColor; LeftCol, RightCol: Integer);
  ...
  begin
    if (FrameSideColors[ASide] <> TColorRec.Null) and
...
begin
  ...
  If (Other = nil) or (Other.FrameColor = TColorRec.Null) then
    exit;
...
begin
  ...
  FFrameSidesInitialized := False;
  for i := low(TLazSynBorderSide) to high(TLazSynBorderSide) do begin
    FFrameSideColors[i] := TColorRec.Null;
...
begin
  Result := (Background <> TColorRec.Null) or (Foreground <> TColorRec.Null) or (FrameColor <> TColorRec.Null) or
            (Style <> []) or (StyleMask <> []);
end;

{ TLazSynSurface }

//function TLazSynSurface.GetHandle: THandle;
//begin
...
function TLazSynSurface.getFBoundsBottom: integer;
begin
  if Assigned(FBounds) then
  result := Round(FBounds.Bottom)
  else
    result := 0;
end;

function TLazSynSurface.getFBoundsLeft: Integer;
begin
  if Assigned(FBounds) then
...
function TLazSynSurface.getFBoundsRight: Integer;
begin
  if Assigned(FBounds) then
  result := Round(FBounds.Right)
  else
    result := 0;
end;

function TLazSynSurface.getFBoundsTop: Integer;
begin
  if Assigned(FBounds) then
  result := Round(FBounds.Top)
  else
    result := 0;
end;

function TLazSynSurface.getRectFromBounds: TRect;
begin
  Result.Left := left;
  Result.Right := Right;
  Result.Top := Top;
...
constructor TLazSynSurface.Create(AOwner: TControl);
begin
  FBounds := TBounds.Create(TrectF.Create(0,0,0,0));
  FOwner := AOwner;
end;
...
begin
  ...
  if (AClip.Left   < Bounds.Left)   then AClip.Left   := Round(Bounds.Left);
  if (AClip.Right  > Bounds.Right)  then AClip.Right  := Round(Bounds.Right);
  if (AClip.Top    < Bounds.Top)    then AClip.Top    := Round(Bounds.Top);
  if (AClip.Bottom > Bounds.Bottom) then AClip.Bottom := Round(Bounds.Bottom);
...
procedure TSynBookMarkOpt.SetBookmarkImages(const Value: TList);
begin
...
begin
  ...
  if InternalImagesUsers = 1 then begin
    InternalImages := TBitmap.Create(0,0);
